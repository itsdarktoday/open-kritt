...
def record_model_output_artifact(
    data_dir: str,
    *,
    scan_id: int,
    metadata_id: int,
    ...
    return _record_model_output(
        ...
        step_id=step_id,
        error=None,
        root_name=MODEL_OUTPUT_DIR,
        limit=MODEL_OUTPUT_LIMIT,
    )
    ...
    *,
    ...
    try:
        with _MODEL_ERROR_OUTPUT_LOCK:
            ...
            written_files = ["stdout.txt", "stderr.txt"]
            if error is not None:
                written_files.append("error.txt")
            reserved = set(written_files) | {"metadata.json"}
            for name, contents in sorted(files.items()):
                safe_name = _safe_file_name(str(name))
                if safe_name in reserved:
            ...
            _write_text(tmp_path / "metadata.json", json.dumps(metadata, indent=2, sort_keys=True))
            tmp_path.rename(final_path)
            _rotate_outputs_with_limit(root, limit)
    except OSError:
        LOGGER.exception("failed to record model output artifact for scan %s metadata %s", scan_id, metadata_id)
        return None
    return str(final_path)


def _rotate_outputs_with_limit(root: Path, limit: int):
    entries = []
    for path in root.iterdir():
        if not path.is_dir() or path.name.startswith("."):
            continue
        try:
            ...
            continue
    entries.sort(reverse=True)
    for _mtime, _name, stale in entries[limit:]:
        shutil.rmtree(stale, ignore_errors=True)
